...
TEST(CheckPIDTest, ZeroBuf) {
  EXPECT_THROW(dv::pidcoll::checkPID(nullptr, 5000), std::invalid_argument);
}

TEST(CheckPIDTest, LessThanMessageSize) {
  EXPECT_THROW(dv::pidcoll::checkPID("1234567", 7), std::invalid_argument);
}
...
TEST(CheckPIDTest, ThreeMessagesNoCollisions) {
  const mock::PidInfos pidInfos = {{0, 11}, {1, 11}, {2, 11}};
  auto data = mock::prepareMessages(pidInfos);
  EXPECT_EQ(11, dv::pidcoll::checkPID(data.data(), data.size()));
}

TEST(CheckPIDTest, ThreeMessages2Collisions) {
  const mock::PidInfos pidInfos = {{0, 11}, {2, 12}, {5, 14}};
  auto data = mock::prepareMessages(pidInfos);
  EXPECT_EQ(14, dv::pidcoll::checkPID(data.data(), data.size()));
}

TEST(PidDiffTest, PidOverflow) {
  EXPECT_THROW(dv::pidcoll::pidDiff(8192, 0), std::runtime_error);
}

TEST(PidDiffTest, PrevPidOverflow) {
  EXPECT_THROW(dv::pidcoll::pidDiff(0, 8192), std::runtime_error);
}

TEST(PidDiffTest, PidIsEqualToPrevPid) {
  EXPECT_THROW(dv::pidcoll::pidDiff(11, 11), std::runtime_error);
}
...
TEST(CalcCCTest, PidDiffGreaterThanMaxPid) {
  EXPECT_THROW(dv::pidcoll::calcCC(8192, 0), std::invalid_argument);
}

TEST(CalcCCTest, PidDiffIsZero) {
  EXPECT_THROW(dv::pidcoll::calcCC(0, 0), std::invalid_argument);
}

TEST(CalcCCTest, PidDiffIsEqualToOne) {
  EXPECT_EQ(7, dv::pidcoll::calcCC(1, 7));
}

TEST(CalcCCTest, CcGreaterThanMaxCc) {
  EXPECT_THROW(dv::pidcoll::calcCC(0, 16), std::invalid_argument);
}
